...
public class Query {
  ...
  // Parse Text File to retrieve Key,Value Pairs where the Keys are store dates i.e. ss_sold_date_sk_1.
  // The values are the net sales associated with the corresponding dates.
        ...
        if (!tokens[0].equals("")) {
          ...
          if ((startDate <= Integer.parseInt(tokens[0])) && (Integer.parseInt(tokens[0]) <= endDate)){
            ...
            if (!tokens[22].equals("")) {
              ...
              // Get the net profit for the date.
              Double profit = Double.parseDouble(tokens[22]); 
              // Get the store date for the record.   
              String _date = tokens[0];
              // Convert primitive Java Types to MapReduce Types.
              Date.set(_date);
              netProfit.set(profit);
              // Write store date and corresponding net profit to reducer.
              context.write(Date, netProfit);
  ...
  // For every key, SUM the array of profits over the dates to achieve net sales for every date.
